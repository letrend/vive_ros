...
Reconstructor::Reconstructor(){
    ...
    if(!inputCapture.isOpened()){
        ROS_ERROR("could not open vive camera, check reconstructor.launch for correct cameraID, and make sure you I have sufficient permissions");
    }
    ...
    if(!fs.isOpened()){
        ROS_ERROR("Could not read intrinsic params from %s, check the reconstructor.launch file and update the filepath!", filepath.c_str());
    }
...
void Reconstructor::Run(int rate) {
    ...
    while (ros::ok()) {
        ...
        Eigen::Affine3d Rt;
        tf::transformTFToEigen(tf_between_frames, Rt);
        Eigen::Matrix3d R_ = Rt.rotation();
        Eigen::Vector3d T_ = Rt.translation();
        ROS_INFO_STREAM("Trafo between frames:\n" << R_ << "\n" << T_);
        Mat R,T;
        eigen2cv(R_, R);
        eigen2cv(T_, T);
        Mat R1, R2, P1, P2, Q;
        stereoRectify(cameraMatrix, distCoeffs, cameraMatrix, distCoeffs, image_size, R, T, R1, R2, P1, P2, Q);
        Mat disparity(img_old.size(), CV_8UC1);

        cvtColor(img_old, img_old_cpy,CV_BGR2GRAY);
        remap(img_old_cpy, img_old_cpy, map1, map2, INTER_LINEAR);

        cvtColor(img_new, img_new_cpy,CV_BGR2GRAY);
        remap(img_new_cpy, img_new_cpy, map1, map2, INTER_LINEAR);

        imshow("img_new", img_new);
        imshow("img_new_rectified", img_new_cpy);

        stereoBM->compute(img_old_cpy, img_new_cpy, disparity);
        ...
        points.id = counter ++;
        points.pose.position.x = T_(0);
        points.pose.position.y = T_(1);
        points.pose.position.z = T_(2);
        ...
        for(int y=0; y<disparity.rows; ++y) {
            for(int x=0; x<disparity.cols; ++x) {
                vec_tmp(0)=x; vec_tmp(1)=y; vec_tmp(2)=disparity.at<float>(y,x); vec_tmp(3)=1;
                vec_tmp = Q*vec_tmp;
                if(fabs(vec_tmp(3))>0)
                    vec_tmp /= vec_tmp(3);
                else
                ...
                p.x = vec_tmp(0)/1000.0;
                p.y = vec_tmp(1)/1000.0;
                p.z = vec_tmp(2)/1000.0;
...
//        imshow("rectified", canvas);
        ...
        swap(img_old,img_new);
        swap(img_old_cpy,img_new_cpy);
        swap(keypoints_old, keypoints_new);
        swap(descriptors_old,descriptors_new);
        swap(transform_old,transform_new);
    }
}
