...
void CQTrackedCameraOpenVRTest::LogMessage( ELogLevel nLogLevel, const char *pMessage, ... )
{
    ...
    snprintf( formattedMessage, sizeof( formattedMessage ), pMessage, args );
...
void CQTrackedCameraOpenVRTest::OnDisplayRefreshTimeout()
{
    ...
    if ( m_VideoSignalTime.elapsed() >= 2000 )
    {
        ...
        ROS_ERROR( "No Video Frames Arriving!\n" );
...
{
    ROS_INFO( "StartVideoPreview()" );

    // Allocate for camera frame buffer requirements
    uint32_t nCameraFrameBufferSize = 0;

    vr::EVRTrackedCameraError error;
    error = m_pVRTrackedCamera->GetCameraFrameSize( vr::k_unTrackedDeviceIndex_Hmd, vr::VRTrackedCameraFrameType_Distorted,
                                                            &m_nCameraFrameWidth, &m_nCameraFrameHeight, &nCameraFrameBufferSize );
    if (  error != vr::VRTrackedCameraError_None )
    {
        bool hasCamera;
        vr::EVRTrackedCameraError error2;
        error2 = m_pVRTrackedCamera->HasCamera( vr::k_unTrackedDeviceIndex_Hmd, &hasCamera );
        vr::HmdVector2_t pFocalLength, pCenter;
        vr::EVRTrackedCameraError error3;
        error3 = m_pVRTrackedCamera->GetCameraIntrinisics( vr::k_unTrackedDeviceIndex_Hmd, vr::VRTrackedCameraFrameType_Undistorted,
                                                  &pFocalLength, &pCenter ) ;
        ROS_ERROR_STREAM( "GetCameraFrameBounds() Failed! " << m_pVRTrackedCamera->GetCameraErrorNameFromEnum(error)
                                                            << " has camera "<< hasCamera <<m_pVRTrackedCamera->GetCameraErrorNameFromEnum(error2)
                                                            << " focal length: " << pFocalLength.v[0] << "," << pFocalLength.v[1]
                                                            << " pCenter: " << pFocalLength.v[0] << "," << pFocalLength.v[1]<<m_pVRTrackedCamera->GetCameraErrorNameFromEnum(error3)
    ...
    while(m_hTrackedCamera == INVALID_TRACKED_CAMERA_HANDLE) {
        vr::EVRTrackedCameraError error4 = m_pVRTrackedCamera->AcquireVideoStreamingService(
                vr::k_unTrackedDeviceIndex_Hmd, &m_hTrackedCamera);
        if (m_hTrackedCamera == INVALID_TRACKED_CAMERA_HANDLE) {
            ROS_ERROR_STREAM("AcquireVideoStreamingService() Failed! "
                                     << m_pVRTrackedCamera->GetCameraErrorNameFromEnum(error4));
            //return false;
        }
        usleep(100000);
    }
...
bool CQTrackedCameraOpenVRTest::InitOpenVR()
{
    ...
    if ( eError != vr::VRInitError_None )
    {
        m_pVRSystem = nullptr;
        char buf[1024];
        ROS_ERROR_STREAM("Unable to init VR runtime: " << vr::VR_GetVRInitErrorAsSymbol( eError ) );
        return false;
    }
    else
    {
        ...
        ROS_INFO("VR HMD: %s %s", systemName, serialNumber );
    }

    m_pVRTrackedCamera = vr::VRTrackedCamera();
    if ( !m_pVRTrackedCamera )
    {
        ROS_ERROR("Unable to get Tracked Camera interface." );
        return false;
    }
    ...
    {
        ROS_ERROR( "No Tracked Camera Available! (%s)", m_pVRTrackedCamera->GetCameraErrorNameFromEnum( nCameraError ) );
        return false;
    }
    ...
    {
        ROS_ERROR( "Failed to get tracked camera firmware description!" );
        return false;
    }

    ROS_INFO("Camera Firmware: %s\n\n", buffer );
